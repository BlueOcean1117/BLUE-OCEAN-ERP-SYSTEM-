...
export default function Step2({ initial = {}, onNext, onPrev, onUpdate }) {
 ...
 useEffect(() => {onUpdate(form);}, [form, onUpdate]);// push initial to parent
      ...
      <div>
        ...
        <div style={{display:'flex', gap:8, marginTop:8}}>
          {form.label_urls?.map((u,i)=>(
            <img key={i} src={u.url || u} style={{width:80,height:60,objectFit:'cover',borderRadius:6}} alt="label" />
          ))}
        </div>
      </div>
